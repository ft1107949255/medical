  ...
  methods: {
    ...
    handleRowClick(val) {
      ...
      if(val.hasOwnProperty('checkItemIds')){
        // this.$refs.formData.focus(this.$refs.formData.form.checkItemIds);
        var that = this;
        setTimeout(function (){
          // 延迟执行，因为modalityTypeName 的watch事件会清空checkItemIds的值。
          that.$refs.formData.form.checkItemIds = val.checkItemIds =='' ? []: val.checkItemIds.split(',');
        }, 1000);
      }
    ...
    getformData: function() {
      ...
      // 开始添加数据
      this.$api.AddYuyueDataApi(form).then(res => {
        console.log('xxxx',res)
        var data = res;
        // 检查项目和设备类型存储在本地
        localStorage.setItem('checkItemIds', form.checkItemIds)
        localStorage.setItem('modalityTypeName', form.modalityTypeName)
        localStorage.setItem('modalityTypeCode', form.modalityTypeCode)
        if (form.id == ''){
          this.$message({
            showClose: true,
            message: "病人添加成功",
    ...
    OkDialogfenzhen: function() {
      if(this.form1.procedureOfficeName === '') {
        this.$message({
          message: '请选择科室',
          type: 'error'
        });
