       ...
       <!-- <div class="footer-title">
        ...
        <ul>
            <li
              ...
              <preview-dicom class="preview-dicom" :imageId="item" :isShowInfo="false"></preview-dicom>
            </li>
        </ul>
...
export default {
  ...
  methods: {
    // 获取预打印列表
    getPrintImage () {
      this.$api.getPrintImage({patientId: this.$route.query.patientId}).then(res => {
        this.imageList = res.data.map(item => `${this.$route.query.cloud ? cloudPath : staticPath}${item}`)
      })
    },
    // Change mode
