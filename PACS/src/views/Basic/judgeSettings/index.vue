...
export default {
  ...
  methods: {
    isSearch() {
      ...
      this.$api.judgeSettingGetAll(params).then(res => {
    ...
    isDelete() {
      ...
      this.$api.judgeSettingDel(params).then(res => {
        if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '删除成功！！',
    ...
    submitData() {
      ...
      if (this.dialogTitle === '新增综合评价管理') {
        // add
        this.$api.judgeSettingAdd(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '添加成功！！',
        ...
        // update
        this.$api.judgeSettingUpdate(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '修改成功！！',
