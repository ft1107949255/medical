...
export default {
  ...
  methods: {
    isSearch() {
      ...
      this.$api.checkbodytypeSettingGetAll(params).then(res => {
    ...
    isDelete() {
      ...
      this.$api.checkbodytypeSettingDel(params).then(res => {
        if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '删除成功！！',
    ...
    submitData() {
      ...
      if (this.dialogTitle === '新增检查部位类型') {
        // add
        this.$api.checkbodytypeSettingAdd(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '添加成功！！',
        ...
        // update
        this.$api.checkbodytypeSettingUpdate(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '修改成功！！',
