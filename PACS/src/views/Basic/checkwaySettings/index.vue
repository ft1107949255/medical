...
export default {
  ...
  methods: {
    isSearch() {
      ...
      this.$api.checkwaySettingGetAll(params).then(res => {
    ...
    isDelete() {
      ...
      this.$api.checkwaySettingDel(params).then(res => {
        if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '删除成功！！',
    ...
    submitData() {
      ...
      if (this.dialogTitle === '新增检查方法') {
        // add
        this.$api.checkwaySettingAdd(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '添加成功！！',
        ...
        // update
        this.$api.checkwaySettingUpdate(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '修改成功！！',
