...
import menuList from './leftMenu.json';
export default {
  components: {
    leftSearch,
    centent
  ...
  methods: {
    ...
    getDepartmentlist(datalist) {  //获取检查科室列表
     this.$http
        .post(this.ip + '/xhhms/rest/interHospitalController/v1/getMyHospitalList', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
    ...
    getCheckItem(datalist){
      this.$http
        .post(this.ip + '/xhhms/rest/interMedicaCheckItemController/v1/getlist', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
          var data = JSON.parse(res.data);
          if (data.status == 1) {
    ...
    getCheckWay(datalist){
      this.$http
        .post(this.ip + '/xhhms/rest/interMedicalCheckTypeController/v1/getlist', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
          var data = JSON.parse(res.data);
          if (data.status == 1) {
            this.BodyTypelist = data.data;
    ...
    getCheckBody(datalist){
      this.$http
        .post(this.ip + '/xhhms/rest/interMedicaBodyTypeController/v1/getlist', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
          var data = JSON.parse(res.data);
          if (data.status == 1) {
            this.BodyTypelist = data.data;
