...
export default {
  ...
  methods: {
    isSearch() {
      ...
      this.$api.cameradirectionSettingGetAll(params).then(res => {
    ...
    isDelete() {
      ...
      this.$api.cameradirectionSettingDel(params).then(res => {
        if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '删除成功！！',
    ...
    submitData() {
      ...
      if (this.dialogTitle === '新增摄影方向') {
        // add
        this.$api.cameradirectionSettingAdd(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '添加成功！！',
        ...
        // update
        this.$api.cameradirectionSettingUpdate(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '修改成功！！',
