...
export default {
  ...
  methods: {
    isSearch() {
      ...
      this.$api.pricetypeSettingGetAll(params).then(res => {
    ...
    isDelete() {
      ...
      this.$api.pricetypeSettingDel(params).then(res => {
        if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '删除成功！！',
    ...
    submitData() {
      ...
      if (this.dialogTitle === '新增费用类型') {
        // add
        this.$api.pricetypeSettingAdd(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '添加成功！！',
        ...
        // update
        this.$api.pricetypeSettingUpdate(params).then(res =>{
          if(res.status === 1 || res.status === '1') {
            this.$message({
              showClose: true,
              message: '修改成功！！',
