...
export default {
  ...
  data() {
    return {
      ...
      // tableApi
      tableApi: {
        list: 'expertsSettingGetAll',
        add: 'expertsSettingAdd',
        update: 'expertsSettingUpdate',
        del: 'expertsSettingDel'
      },
  ...
  methods: {
    ...
    handleSave() {
      ...
      this.$refs.formList.validate().then(res => {
        if (res) {
          this.currentRow.office = Array.isArray(this.currentRow.office) ? this.currentRow.office.pop() : this.currentRow.office
    ...
    handleEdit() {
      ...
      let cache = Object.assign({}, this.selectArr[0])
      // cache.office = []
      // this.currentRow = cache
      this.getOfficeId(cache.office, (data) => {
        // this.$set(this.currentRow, 'office', data)
        cache.office = data
        console.log(JSON.stringify(cache, '', 2))
        this.currentRow = cache
        console.log(JSON.stringify(this.currentRow, '', 2))
        this.formTitle = '编辑'
        this.formMode = 'edit'
    ...
    // 获取专家科室
    getOfficeId (id, callback) {
      let arr = []
      mapList(this.officeList)
    ...
    getOfficeList() {
      ...
      this.$api.getHdListById(params).then(res => {
