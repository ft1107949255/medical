<template>
  <div class="left-search" style="font-size:12px; !important">
    ...
    <inputs class="padding" name='病人ID'  @change="change2" ref="patientId"/>
  ...
  export default {
    ...
    methods: {
      ...
      _getHospitalList() {
        ...
        // )
        this.$api.LeftSearchgetHospitalList(params).then((res) => {
          var data = res;
          if (data.status == 1) {
              data.data.map(item => {
      ...
      _getDeviceTypeList() {
        ...
        //   { headers: { 'X-AUTH-TOKEN': this.token }}
        // )
        this.$api.LeftSearchgetDeviceTypeList(params).then((res) => {
          var data = res;
          if (data.status == 1) {
      ...
      _getCheckItemList() {
        ...
        //   { headers: { 'X-AUTH-TOKEN': this.token }}
        // )
        this.$api.LeftSearchgetCheckItemList(params).then((res) => {
          var data = res;
          if (data.status == 1) {
      ...
      change6(value) {
        var data = value.split(',');
        this.searchParams.procedureCode = data[0];
        this.searchParams.procedurename = data[1];
      },
      change7(value) {
