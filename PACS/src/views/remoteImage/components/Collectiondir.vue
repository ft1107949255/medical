<template>
  <div class="Collection_centents">
    ...
    <div class="Collection_tree" style="width:100%; height:460px;" >
       <ul id="shuyuSelect" class="ztree" style=" width: 90%; cursor: pointer; " ></ul>
    </div>
  </div>
</template>
...
import index from '../../components/treeTable/utils';
export default {
  data() {
    return {
      msg: "",
  ...
  methods: {
    getCollectionList(){
      ...
      //   }
      // })
      this.$api.IndexgetCollectionlist(params).then((res) => {
        var data = res;
        console.log(data);
        this.rowdata={};
    ...
    shuyuOnExpand(event, treeId, treeNode){
          ...
          params.collectFloderId=treeNodeId
          // this.$http.post(this.ip + '/xhhms/rest/interCollectController/v1/getlistByPid', params, {
          //   headers: {
          //        'X-AUTH-TOKEN': this.token
          //   }
          // })
          this.$api.IndexgetCollectionlistById(params).then((res) => {
            var data = res;
            console.log(res);
            if (!!data&&data.status=="1") {
        ...
        params.id=this.rowdata.id;
            ...
            if(!this.rowdata.id){
              this.$message({
                ...
                this.$api.IndexdelFolder.then((res) => {
                  var data = res;
                  if (data.status == 1) {
                      this.$message({
                        showClose: true,
