...
export default {
  ...
  methods: {
     getlist() { // 获取检查类型
      console.log("JINLAI");
      this.$http
        .post(this.ip + '/xhhms/rest/interMedicalDeviceTypeController/v1/getlist', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
          console.log(res);
    ...
    getEquipmentlist() {  //获取检查设备列表
      console.log("JINLAI");
      this.$http
        .post(this.ip + '/xhhms/rest/interMedicalDeviceController/v1/getlist', { headers: { "X-AUTH-TOKEN": this.token } })
        .then(res => {
          console.log(res);
          var data = JSON.parse(res.data);
          if (data.status == 1) {
