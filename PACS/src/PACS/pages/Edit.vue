...
<template>
  <div class="edit">
      ...
      <div class="centent">
        <div class="left">
          <div class="left_tree">
            <div class="left_header"><span style="color:#606266;font-size:12px;">术语</span></div>
            <!--s-->
            <!--tree1 :data="knowledgetreedata" :defaultProps="defaultProps" @getTreeId="getTreeId"/-->
            <div  style="cursor: pointer;min-height: 200px; max-height:300px; overflow-y: auto; overflow-x:hidden;" >
        ...
        <el-dialog
          ...
          <div v-if="zhenduanreportList.length >0">
            <div style="width:100%; height:400px;  overflow-y: auto; overflow-x:hidden;">
              ...
              <div  style="width:98%; margin: 10px auto; border-top:1px solid #ffff; padding-left:2%;" v-for="(item,index) in zhenduanreportList" v-bind:key="index">
                <div style="margin-top: 10px;">
                  <span>记录{{index +1}}</span>
                  <span>修改医生：{{item.createName}}</span>
        ...
        <el-dialog
          ...
          <span slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="OkDialogReportBeizhu">发 送</el-button>
            </span>
        </el-dialog>
        <el-dialog
          title="警告"
...
<script>
  ...
  import tree1 from './../components/Trees1.vue';
  import editCentent from "./../components/EditCentent.vue";
  import page from './../components/table/Pagination'
  import editkuang from './../components/Editkuang.vue'
  ...
  import knowledgeright from "./../components/knowledgeRight.vue";
  ...
  export default {
      ...
      window.addEventListener('message',function(res){
        ...
        if(res.data && !res.data.hasOwnProperty('type')){
          ...
          if(data.doSave){
            ...
            that.$http.post(that.ip + '/xhhms/rest/interRemoteReportController/v1/doSave', params, {
              headers: {
      ...
      var params=new Object();
          this.$http.post(this.ip + '/xhhms/rest/interRemoteReportController/v1/getKnowledge', params, {
            ...
            if (!!data&&data.status=="1") {
              ...
              //treeObj.onExpand(nodes[0]);
              //this.shuyuOnClick(event, 0, nodes);
    ...
    methods:{
      ...
      shuyuOnClick(event, treeId, treeNode){
        if(!treeNode.isParent){   // 判断当前节点不是父级节点
      ...
      save: function () {
        ...
        if(form.acrIdLeft !=='' && form.acrIdLeft !=='undefined'){
          params.acrIdLeft = form.acrIdLeft;
          params.acrIdRight = form.acrIdRight;
        }
        console.log(params);
      ...
      chargeReport: function () {
        ...
        this.$http.post(this.ip + '/xhhms/rest/interRemoteReportController/v1/diagnosticrecord', params, {
          headers: {'X-AUTH-TOKEN': localStorage.getItem('token') }
        }).then((res) => {
          var data = JSON.parse(res.data);
          if (!!data&&data.status=="1") {
            console.log('诊断记录---',data);
            this.zhenduanreportList=[];
            this.zhenduantotal = data.total;
            data.data.map(item=>{
              console.log(item.imageDescribe);
              if(typeof(item.imageDescribe) !='undefined' && typeof(item.conclusion) !='undefined' && item.conclusion !='' && item.imageDescribe !=''  ){
                item.imageDescribe = unescape(item.imageDescribe);
                item.conclusion = unescape(item.conclusion);
                let d = new Date(item.createDate);
                item.createDate = d.getFullYear() + '-' + (d.getMonth() + 1 > 9 ? d.getMonth() + 1 : '0' + (d.getMonth() + 1)) + '-' + (d.getDate() > 9 ? d.getDate() : '0' + d.getDate());
      ...
      seeMedia: function () {
        // window.open('/#/patientimages','newwindow','width='+ (window.screen.availWidth-10)+',height='+(window.screen.availHeight-30)+ ',top=0,left=0,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no')
        ...
        this.$http.post(this.ip+'/xhhms/rest/interRemoteReportController/v1/getReportByid',params,{
          ...
          if(data.status === '1' && data.data){
            ...
            // }
              ...
              //console.log(typeof(data.data.remoteStatus));
              this.remoteStatus = typeof(data.data.remoteStatus)=='undefined' ? 0: data.data.remoteStatus;
